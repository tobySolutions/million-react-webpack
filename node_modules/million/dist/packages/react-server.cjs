...
const HTMLReactParser = /*@__PURE__*/getDefaultExportFromCjs(lib$8);
...
const SSRContext = React__default.createContext(defaultContext);
const IsSSRContext = React__default.createContext(false);
const canUseDOM = Boolean(
  typeof window !== "undefined" && window.document && window.document.createElement
);
const componentIds = /* @__PURE__ */ new WeakMap();
function useCounter(isDisabled = false) {
  const ctx = React$1.useContext(SSRContext);
  const ref = React$1.useRef(null);
  if (ref.current === null && !isDisabled) {
    const currentOwner = React__default.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED?.ReactCurrentOwner?.current;
    if (currentOwner) {
...
function useLegacySSRSafeId(defaultId) {
  ...
  const prefix = ctx === defaultContext && process.env.NODE_ENV === "test" ? "million" : `million${ctx.prefix}`;
  return defaultId || `${prefix}-${counter}`;
}
function useModernSSRSafeId(defaultId) {
  const id = React__default.useId();
  const [didSSR] = React$1.useState(useIsSSR());
  const prefix = didSSR || process.env.NODE_ENV === "test" ? "million" : `million${defaultContext.prefix}`;
...
function useIsSSR() {
  ...
  return React$1.useContext(IsSSRContext);
}

const useLayoutEffect = typeof window === "undefined" ? React$1.useEffect : React$1.useLayoutEffect;
let globalInfo;
const block = (Component, options = {}) => {
  ...
  const rscBoundary = options.rsc ? createRSCBoundary(Component, noSlot, options.svg) : null;
  function MillionBlockLoader(props) {
    const container = itsFine.useContainer();
    const parentRef = itsFine.useNearestParent();
    const id = useSSRSafeId();
    const initializedRef = React$1.useRef(false);
    const ref = React$1.useRef(null);
    const patch = React$1.useRef(null);
    const effect = React$1.useCallback(() => {
      const init = () => {
  ...
  The block element should be the only child of the \`${ref.current.cloneNode().outerHTML}\` element.
  To avoid this error, \`experimental_options.noSlot\` should be false`)
    ...
    patch.current?.(props);
    const vnode = React$1.createElement(
      React$1.Fragment,
      null,
      React$1.createElement(Effect, { effect }),
      rscBoundary ? React$1.createElement(rscBoundary, { ...props, ref }) : createSSRBoundary(
        Component,
        props,
        ref,
        noSlot,
...
function For({ each, children, ssr, svg }) {
  const [ready, setReady] = React$1.useState(Boolean(globalInfo));
  React$1.useEffect(() => {
    if (!globalInfo) {
      importSource(() => {
        setReady(true);
  ...
  if (!ready || !globalInfo) {
    ...
    return React$1.createElement(
      svg ? itsFine.SVG_RENDER_SCOPE : itsFine.RENDER_SCOPE,
      { suppressHydrationWarning: true },
      ...each.map(children)
    );
  }
  return React$1.createElement(globalInfo.For, {
    each,
    children,
    ssr,
    svg
...
const ssrElementsMap = /* @__PURE__ */ new Map();
const createSSRBoundary = (Component, props, ref, noSlot, id, svg = false) => {
  ...
  const el = React$1.createElement(svg ? itsFine.SVG_RENDER_SCOPE : itsFine.RENDER_SCOPE, {
    suppressHydrationWarning: true,
    ref,
    id,
    ...ssrProps
...
const createHydrationBoundary = (id, phase, isSSR) => {
  return isSSR ? React$1.createElement("template", { id: `${phase}-${id}` }) : null;
};
const createRSCBoundary = (Component, noSlot, svg = false) => {
  return React$1.memo(
...
function compiledBlock(render, { portals, ...options }) {
  ...
  const Component = portals && portalCount > 0 ? (props) => {
    const id = useSSRSafeId();
    const [current] = React$1.useState(() => []);
    const [firstRender, setFirstRender] = React$1.useState(true);
    const derived = { ...props };
    for (let i = 0; i < portalCount; i++) {
      const index = portals[i];
      derived[index] = itsFine.renderReactScope(
        derived[index],
        false,
        current,
        i,
    ...
    return React$1.createElement(
      React$1.Fragment,
      {},
      React$1.createElement(RenderBlock, derived),
      !firstRender ? targets : void 0
    );
  } : (props) => React$1.createElement(RenderBlock, props);
  if (options.name) {
